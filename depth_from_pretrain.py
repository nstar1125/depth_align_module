...
import tyro
from utils.camera_utils import euclidean_to_z_depth
...
from utils.misc import torch_compile
...
class DepthsFromPretrained:
    ...
    def main(self):
        if os.path.exists(os.path.join(self.data_dir, "transforms.json")):
            ...
            print(
                f"Found images in /{self.img_dir_name}, using these to generate monodepths."
            )
...
def depth_align(
    ...
    print(
        f"Average depth completion error for batch depths is: {avg:3f} which is {'good' if avg<0.2 else 'bad'}"
    )
...
def run_monocular_depths(
    images: List[Path],
    save_path: Optional[Path] = None,
    pretrain_model: Literal["zoe, depth_anything_v2"] = "zoe",
):
    image_tensors = []
    ...
    elif pretrain_model == "depth_anything_v2":
        depth_v2 = pipeline(task="depth-estimation", model="depth-anything/Depth-Anything-V2-Small-hf")
    else:
        raise NotImplementedError
...
def depth_from_pretrain(
    ...
    if path_to_transforms is not None:
        ...
        if pretrain_model == "zoe":
            ...
            depth_v2 = pipeline(task="depth-estimation", model="depth-anything/Depth-Anything-V2-Large-hf", device=0)
        else:
            raise NotImplementedError
            ...
            with torch.no_grad():
                ...
                for i in track(
                    ...
                    if pretrain_model == "zoe":
                        ...
                        depth_image = depth_v2(pil_img)["depth"]
                        transform = transforms.ToTensor()
                        mono_depth_tensor = transform(depth_image).permute(1, 2, 0)[..., :3]
                        mono_depth_tensor = mono_depth_tensor.to(device)
                    else:
                        raise NotImplementedError
                    mono_depths.append(mono_depth_tensor)
                mono_depths = torch.stack(mono_depths, dim=0)
    ...
    if create_new_transforms:
        ...
        print(
            f"saving mono_depth_transformations.json to {input_folder}/mono_depth_transformations.json"
        )
        with open(
