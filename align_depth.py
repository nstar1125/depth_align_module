...
import tyro
import os
from depth_from_pretrain import depth_from_pretrain
from utils.camera_utils import project_pix
from utils.utils import depth_path_to_tensor, get_filename_list, save_depth
from rich.progress import track
from torch import Tensor


#from utils.colmap_loader import qvec2rotmat, read_extrinsics_binary, read_extrinsics_text, read_intrinsics_binary, read_intrinsics_text, read_points3D_binary, read_points3D_text
from utils.colmap_parsing_utils import qvec2rotmat, read_cameras_binary, read_cameras_text, read_images_binary, read_images_text, read_points3D_binary, read_points3D_text
...
class ColmapToAlignedMonoDepths:
    ...
    mono_depth_network: Literal["zoe", "depth_anything_v2"] = "zoe"
    ...
    def main(self) -> None:
        ...
        if not self.skip_colmap_to_depths:
            print("Generating sfm depth maps from sparse colmap reconstruction")
        ...
        if not self.skip_mono_depth_creation:
            print("Computing mono depth estimates")
            if not (self.data / Path("mono_depth")).exists() or True:
                ...
                print("Found previous /mono_depth path")

        print("Aligning sparse depth maps with mono estimates")
...
def colmap_sfm_points_to_depths(
    ...
    if (recon_dir / "points3D.bin").exists():
        ...
        #cam_id_to_camera = read_intrinsics_binary(recon_dir / "cameras.bin")
        #im_id_to_image = read_extrinsics_binary(recon_dir / "images.bin")
        ...
        #cam_id_to_camera = read_intrinsics_text(recon_dir / "cameras.txt")
        #im_id_to_image = read_extrinsics_text(recon_dir / "images.txt")
    ...
    for im_id, im_data in track(iter_images, description="..."):
        ...
        xyz_world = np.array([ptid_to_info[pid].xyz for pid in pids]) # shape = (p_id, 3)
        # delete
        # xyz_world = np.array([p.xyz for p in ptid_to_info.values()])
        rotation = qvec2rotmat(im_data.qvec)

        z = (rotation @ xyz_world.T)[-1] + im_data.tvec[-1] # 각 point의 깊이인듯
        ...
        uu, vv = uv[:, 0].astype(int), uv[:, 1].astype(int) #width, height

        depth = np.zeros((H, W), dtype=np.float32)
        depth[vv, uu] = z # 1080, 1920
...
def sdfstudio_alignment(input_dir: Path, iterations: int = 1000):
    ...
    for batch_index in range(0, len(meta["frames"]), batch_size):
        print(
            f"Processing batch {batch_index // batch_size} / {num_frames//batch_size}"
        )
